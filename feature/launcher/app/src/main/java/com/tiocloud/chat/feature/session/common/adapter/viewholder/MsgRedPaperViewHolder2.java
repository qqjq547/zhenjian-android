...
import com.tiocloud.chat.feature.session.common.adapter.viewholder.viewmodel.RedPaperViewModel2;
...
public class MsgRedPaperViewHolder2 extends MsgBaseViewHolder {
    ...
    private final RedPaperViewModel2 viewModel = new RedPaperViewModel2();
    private iRedPaperDialog redPaperDialog;

    public MsgRedPaperViewHolder2(MsgAdapter adapter) {
        super(adapter);
    }
    ...
    private void initUI() {
        ...
        String status = msgRed.getStatus_newPay2payEase();
        Log.d("===红包状态==",message.getUid()+"=="+status+"==");
        ...
        if ("SUCCESS".equals(status)) {
            ...
            if(msgRed.uids!=null){
//                Log.d("===红包状态==",message.getUid()+"=="+status+"=="+msgRed.uids.size()+"=="+msgRed.uids.toString());

                for (int i=0;i<msgRed.uids.size();i++){
                    if(message.getUid().equals(msgRed.uids.get(i))){
                        // 自己已领取
                        setRedStatus(1);
                    }else {
                        // 抢红包中
    ...
    protected View.OnClickListener onContentClick() {
        return v -> {
            ...
            viewModel.getRedStatus(msgRed.rid, this, true);
        };
    }
    ...
    public void onRedStatusResp(int status, boolean showDialog) {
        ...
        iRedPaperVo redPaperVo = new iRedPaperVo(avatar, name, msgRed.text, isSendMsg, msgRed.rid);
        ...
        if (status == 1) {
            ...
            if (isSendMsg && p2PMsg) {
                ...
                redPaperDialog.setOnRedPaperListener(() -> viewModel.getGrabRedPacket(MsgRedPaperViewHolder2.this, redPaperVo.serialNumber));
                redPaperDialog.show();
            }
        }
    }
    ...
    public void onGrabRedPacketResp(PayGrabRedPacketResp resp) {
        ...
        // 跳转红包详情页
        TioWallet.getWallet().PaperDetailActivity_start(getContext(), msgRed.rid);
        // 查询红包状态
        viewModel.getRedStatus(msgRed.rid, this, false);
    }
}
